...
def test_hybrid_search_basic(user_client, test_data):
    ...
    """
    results = user_client.hybrid_search.search(query=test_data["question"])
...
def test_hybrid_search_extended(user_client, test_data):
    ...
    """
    results = user_client.hybrid_search.search(
...
def test_hybrid_search_validation_errors(user_client):
    ...
    with pytest.raises(ValueError, match="query.*string"):
        user_client.hybrid_search.search(query=123)

    # Test empty query
    with pytest.raises(ValueError, match="query.*empty"):
        user_client.hybrid_search.search(query="   ")

    # Test query too long
    with pytest.raises(ValueError, match="query.*100 characters"):
        user_client.hybrid_search.search(query="x" * 101)
